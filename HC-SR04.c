...
void GPIOInit(void) 
{
  ...
  PORTA->PCR[11] |= PORT_PCR_MUX(1);    		//using PTA11 as gpio
	PORTA->PCR[11] |= PORT_PCR_IRQC(0b1001);	//interrupt on rising edge
;
	PTA->PDDR |= (1UL << 10);    //Output pin.  
  PTA->PDDR &= ~(1UL << 11);   //Input pin.
 // PTA->PCOR = (1UL << 10);
	
	NVIC_ClearPendingIRQ(PORTA_IRQn);				/* Clear NVIC any pending interrupts on PORTC_D */
	NVIC_EnableIRQ(PORTA_IRQn);							/* Enable NVIC interrupts source for PORTC_D module */
